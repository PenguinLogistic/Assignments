...
#IMPLEMENT
    ...
    if not newVar:
        for c in csp.get_all_cons():
            if len(c.get_scope()) == 1 or c.get_n_unasgn() == 1:            
                var = c.get_unasgn_vars()
                vals = var[0].cur_domain()
                avars = c.get_scope()
                index = -1
                alist = []
                for variable in avars:
                    if variable.is_assigned():
                        alist.append(variable.get_assigned_value())
                    else:
                        alist.append("notAssigned")
                        index = alist.index("notAssigned")
                for d in vals:
                    alist[index] = d
                    if not c.check(alist):
                        var[0].prune_value(d)
                        pruned.append((var[0], d))
        ...
        for c in csp.get_cons_with_var(newVar):
            if c.get_n_unasgn() == 1:
                ...
                vals = vars[0].cur_domain()
                avars = c.get_scope() #prep for assigned value list
                alist = []
                index = -1
                for variable in avars:
                    if variable.is_assigned():
                ...
                for d in vals:
                    alist[index] = d
                    if not c.check(alist):
                        vars[0].prune_value(d)
                        pruned.append((vars[0], d))
                ...
                for d in v.cur_domain():
                    if not cons.has_support(v,d):
                        ...
                        if v.cur_domain_size() == 0:
                            ...
                            for x in csp.get_cons_with_var(v):
                                if x not in gacQ:
                                    gacQ.append(x)
    else:

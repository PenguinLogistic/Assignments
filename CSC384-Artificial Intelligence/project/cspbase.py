...
class Constraint: 
    ...
    def __init__(self, name, scope, sections): 
        ...
        self.sections_indexes = sections
    ...
    def check(self, vals):
        ...
        count = 0
        length = len(self.sections_indexes)
        prev = 0
        pairs = []

        while count < length:

          pairs.append([prev, self.sections_indexes[count]])

          prev = self.sections_indexes[count]
          count += 1

        for pair in pairs:
          if self.check_helper(pair, vals):
            return True

        return False

    def check_helper(self, pair, vals):
        
      count = 0

      while count < len(vals):
        if count >= pair[0] and count < pair[1]:
          if vals[count] != self.name:
            return False
        else:
          if vals[count] == self.name:
            return False
...
class BT:
    ...
    def print_output(self):
      ...
      hour_count = 0
      variable_array = self.csp.get_all_vars()
      ...
      while hour_count < 24:
        ...
        while day_count < 5:
          ...
          for var in variable_array:
            ...
            var_name = var.name
            var_day = var_name[0:1]
            var_hour = var_name[2:4]

            if day_count == 0: #M

              if var_day == "M":

                if int(var_hour) == hour_count:

                  val = var.get_assigned_value()
                  if val == "NOCOURSE":
                    hour_string += "     |"
                  else:
                    hour_string += " " + var.get_assigned_value() + " |"
